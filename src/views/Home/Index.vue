...
const getNotice = async () => {
  try {
    const res = await NoticeApi.getNoticePage({ pageSize: 10, pageNo: 1, status: 0 } as any)
    res.list.forEach(item => {
      notice.push({
        title: item.title,
        type: item.type === 1 ? '通知' : '公告',
        keys: [item.title.split('：')[0]],
